...
class keyword_usertag_sorting():
    ...
    def sortingData(self, sampleNum: int = 20, challengeNum: int = 10):
        ...
        for uuid in self.uuidData:
            abandonData = {}
            self.uuidData[uuid]['keywordList'], abandonData['keywordList'] = self.uuidData[uuid]['keywordList'][
                                                                             :self.keywordLimit], self.uuidData[uuid][
                                                                                                      'keywordList'][
                                                                                                  self.keywordLimit:self.keywordLimit + challengeNum]
            self.uuidData[uuid]['keywordFreq'], abandonData['keywordFreq'] = self.uuidData[uuid]['keywordFreq'][
                                                                             :self.keywordLimit], self.uuidData[uuid][
                                                                                                      'keywordFreq'][
                                                                                                  self.keywordLimit:self.keywordLimit + challengeNum]
            if len(abandonData['keywordList']) > 0:
                ...
                abandonScore = [keywordValue[keyword] if keyword in keywordValue else 0 for keyword in
                                abandonData['keywordList']]
    ...
    def dealExistedUUID(self):
        ...
        invalidUUIDs = []
        for uuid in self.uuidData:
            if len(self.uuidData[uuid]['keywordList']) == 0:
        ...
        for uuid in invalidUUIDs: del self.uuidData[uuid]
        ...
        for uuid in existed_uuidData:
            ...
            keywordFreqTuple = [item for item in
                                zip(*[self.uuidData[uuid]['keywordList'], self.uuidData[uuid]['keywordFreq']])]
    ...
    def fetch_keywords_value(self):
        query = \
            ...
            FROM keyword_value
            """
        keywordValue = {item[0]: int(item[1]) for item in
                        MySqlHelper('rhea_web_push', is_ssh=True).ExecuteSelect(query)}
        return keywordValue
