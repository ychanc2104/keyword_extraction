...
@timing
def fetch_browse_record2(web_id, datetime_start, datetime_end, is_df=False):
    # date_start = get_yesterday(is_UTC0=is_UTC0)
    # date_end = get_today(is_UTC0=is_UTC0) - datetime.timedelta(seconds=1)
    query = f"""
            SELECT 
...
def main_keyword_uuid(web_id, date, jieba_base, stopwords, stopwords_usertag):
    ...
    for i in range(12):  ## every two hours
        ...
        # df_statistics[['web_id', 'hour', 'date']] = [[web_id, hour, date]] * df_statistics.shape[0]
        ...
        DBhelper('missioner', is_ssh=True).ExecuteUpdate(query, df_usertag_save.to_dict('records'))

        # df_statistics_save = df_statistics.query(f"num_usertag<32767")
        # query_stat = MySqlHelper.generate_update_SQLquery(df_statistics_save, 'usertag_uuid_stat')
        # MySqlHelper('missioner', is_ssh=True).ExecuteUpdate(query_stat, df_statistics_save.to_dict('records'))
    df_uuid_stat_all = collect_usertag_uuid_stat(web_id, datetime_list).query(f"num_usertag<32767") ## smallint in SQL
    query_stat = DBhelper.generate_insertDup_SQLquery(df_uuid_stat_all, 'usertag_uuid_stat', ['num_usertag'])
    DBhelper.ExecuteUpdatebyChunk(df_uuid_stat_all, 'missioner', 'usertag_uuid_stat', query=query_stat)

    # DBhelper('missioner', is_ssh=True).ExecuteUpdate(query_stat, df_uuid_stat_all.to_dict('records'))
...
if __name__ == '__main__':
    ...
    # web_id = 'mirrormedia'
    # data = fetch_browse_record(web_id, datetime_start='2022-04-14 00:00:00', datetime_end='2022-04-14 01:00:00',
    #                            is_df=False)
    # data2 = fetch_browse_record2(web_id, datetime_start='2022-04-14 00:00:00', datetime_end='2022-04-14 01:00:00',
    #                            is_df=False)

    # web_id = 'mirrormedia'
    # # data_dict = fetch_date_usertag_meet_criteria(web_id, n_limit=100)
    # # date_dict2 = sorting_data_dict(data_dict)
    ...
    # web_id = 'mirrormedia'
    # date = '2022-04-14'
    # # datetime_list = datetime_range(date, num_days=1, hour_sep=2)[:-1]
    # # df_uuid_stat_all = collect_usertag_uuid_stat(web_id, datetime_list)
    #
    #
    # data_dict = fetch_date_usertag_meet_criteria(web_id, n_limit=100)
    # date_dict2 = sorting_data_dict(data_dict)
